...
from nuclei_segmenter.vis import relabel_by, plot_all, plot_hist, load_full_df, plot_hist_per_sample, \
    plot_full_size_hist, plot_normalized_size_hist, plot_classification, plot_stackedbar_classification
...
def small_report(segment_path, excel_writer):
    ...
    filepath = DATA_DIR / segment_path.parent.stem / (segment_path.stem + '.czi')
    ...
    with PdfPages(segment_path.with_suffix('.pdf')) as pp:
        ...
        nuclei_props = nuclei_props[nuclei_props['z'].isin(good_planes)].query('area > 5')
        plot_hist(nuclei_props.area.values, max_size=max_size, bin_width=bin_width)
        pp.savefig()
        plt.close()

        print('Saving big excel file')
        sheet_name = segment_path.stem.replace(' repeat EdU DAPI', '')
        nuclei_props.to_excel(excel_writer, sheet_name=sheet_name)
...
def summary_report(nuclei_size_excel_path):
    ...
    model_2 = get_gmm(df_2.area.values, number_of_classes=2)
    ...
    df_2['predicted'] = df_2.area.apply(lambda x: 'big' if x > 150 else 'small')
    with pd.ExcelWriter(nuclei_size_excel_path.with_name('using_150_area.xlsx')) as writer:
        save_summary_tables(df_2, writer)

    df_2['predicted'] = df_2.area.apply(lambda x: 'big' if x > 200 else 'small')
    with pd.ExcelWriter(nuclei_size_excel_path.with_name(
            'using_200_area.xlsx')) as writer:
        save_summary_tables(df_2, writer)
...
def add_plots_to_pdf(df, model, pp):
    ...
    plot_normalized_size_hist(df)
    pp.savefig()
    plt.close()

    plot_normalized_size_hist(df, log_y=True)
    pp.savefig()
    plt.close()
    ...
    plot_stackedbar_classification(df, ['condition', 'region'], normalize=False)
    plt.tight_layout()
    pp.savefig()
    plt.close()
